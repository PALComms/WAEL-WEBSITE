...
import { getBlogPost, getAllBlogSlugs } from "@/sanity/sanity-utils";
...
export async function generateMetadata({ params: promise }) {
  ...
  return {
    title: `${post.title} | WAEL Blog`,
    description:
      post.excerpt || (post.content && post.content.substring(0, 160) + "..."),
    keywords: post.tags || ["technology", "workplace", "innovation"],
    authors: [{ name: post.author?.name || "Wael Blog" }],
    openGraph: {
      title: post.title,
      description:
        post.excerpt ||
      ...
      authors: [post.author?.name || "Wael Blog"],
